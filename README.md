...
리소스 카탈로그, 접근 정책, 자격증명, IAM을 한곳에서 운영하는 Next.js Access Governance 애플리케이션입니다. 현재 저장소는 업무 화면과 권한 기반 UI를 검증하는 foundation이며, 실제 인증·백엔드 연동은 포함하지 않습니다.
...
- `David`: Access Governance 시스템 관리자
- `Emma`: 개발 2팀 조직장(소유 조직의 서비스·엔드포인트 운영)
- `Owen`: Access Governance 정책 운영자·Access Governance UI 리소스 관리자, 개발 1팀 경유 IAM 운영자
- `개발 1팀`: Access Governance IAM 운영자
- `Daniel`: IAM 영역이 제외된 Access Governance 일반 사용자 권한 확인용
