...
| IAM             | [사용자](users.md), [조직](organizations.md), [역할](roles.md), [어플리케이션](applications.md) | IAM 운영자, 시스템 관리자                |
| 서비스 카탈로그 | [서비스](services.md), [엔드포인트](service-endpoints.md)                                       | 서비스 운영자, 조직장, 시스템 관리자     |
| UI 카탈로그     | [네임스페이스](namespaces.md), [UI 리소스](ui-resources.md)                                     | UI 리소스 관리자, 시스템 관리자          |
...
| COM-SCN-02 | Daniel, IAM UI 정책 미보유          | IAM 메뉴가 LSB에서 사라지고 `/users` 직접 접근은 403을 표시한다.                          | `COM-016~017`, `COM-025`, `USR-006`            | route gate와 API가 같은 권한 결과로 접근을 거부하며 빈 데이터 fallback을 반환하지 않는다 (`COM-S003`)   | ![403 화면](assets/common-access-denied.png) |
