...
| NSP-SCN-05 | 일반 namespace와 폐기 action 권한 | 하위 리소스 영향 확인 후 namespace를 폐기한다. Access Governance 기본 namespace는 폐기하지 못한다. | `UIR-020`                       | 기본 namespace 보호, 하위 UI 리소스 일괄 비활성화와 감사 (`UIR-S008`, `AUD-S002~004`)                                | ![상세 action](assets/namespaces-detail.png)    |
