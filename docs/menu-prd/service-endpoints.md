...
| EPT-SCN-04 | 상세 수정 action·update view 권한                          | 수정 페이지에서 엔드포인트와 정규화 필드를 입력하고 필드·정책·자격증명 영향을 검토한 뒤 저장해 상세로 이동한다.                              | `EPT-010`, `EPT-012`, `COM-026`     | 동일 위치+JSONPath UUID 유지, 필드 diff, 정책·자격증명 영향과 버전 이력 (`EPT-S005`, `EPT-S007`)          | ![상세 action](assets/endpoints-detail.png) |
