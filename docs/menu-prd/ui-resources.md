...
- routes: `/ui-resources`, `/ui-resources/{uiResourceId}`, `/ui-resources/sync`
- 주 사용자: 대상 네임스페이스 관리 역할 보유자, UI 리소스 관리자, 시스템 관리자
- 원본: UI `UIR-003~022`, 서버 `UIR-S001~010`
...
| UIR-SCN-08 | 상세 view와 대상 namespace 관리 범위                   | 목록 행으로 상세에 진입해 상·하위 관계와 이 UI 리소스를 참조하는 정책을 조회하고 정상 상태·정책 참조·하위 리소스 삭제 차단 사유를 함께 확인한다. 고아이며 참조와 하위가 없을 때만 개별 삭제한다. | `UIR-003`, `UIR-022`                   | 관리 범위 기반 상세, 참조 정책·직계 자식 별도 query, 고아·참조 무결성·삭제 action 재검증과 감사 (`UIR-S010`) | ![UI 리소스 상세](assets/ui-resources-detail.png) |
